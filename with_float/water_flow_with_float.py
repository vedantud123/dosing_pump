...
RELAY3 = 5  
BUTTON_PIN = 25
RELAY3_ON_SECONDS = 600
# Float switches (change pins if your wiring is different)
ACID_FLOAT_PIN = 27
CHLORINE_FLOAT_PIN = 22
calibrationFactor = 0.218
...
GPIO.setup(ACID_FLOAT_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
GPIO.setup(CHLORINE_FLOAT_PIN, GPIO.IN, pull_up_down=GPIO.PUD_UP)
...
button_low_since = 0.0
BUTTON_DEBOUNCE_SECONDS = 0.08
BUTTON_STARTUP_GUARD_SECONDS = 3.0
...
def is_acid_level_ok():
    # PUD_UP assumed: LOW means float triggered (low level), HIGH means OK
    return GPIO.input(ACID_FLOAT_PIN) == GPIO.HIGH

def is_chlorine_level_ok():
    # PUD_UP assumed: LOW means float triggered (low level), HIGH means OK
    return GPIO.input(CHLORINE_FLOAT_PIN) == GPIO.HIGH
...
def toggle_relay1():
    ...
    if not relay1_manual and not is_acid_level_ok():
        relay1_manual = False
        acid_pump_running = False
        acid_off_time = 0
        GPIO.output(RELAY1, RELAY_OFF)
...
def toggle_relay2():
    ...
    if not relay2_manual and not is_chlorine_level_ok():
        relay2_manual = False
        chlorine_pump_running = False
        chlorine_off_time = 0
        GPIO.output(RELAY2, RELAY_OFF)
...
def update_loop():
    ...
    global relay1_manual, relay2_manual
    global relay3_running, relay3_off_time, button_prev_state, button_low_since, button_press_armed
    ...
    if now - last_time >= 1:
        ...
        if effective_acid_trigger > 0 and pulses > 0 and is_acid_level_ok():
        ...
        if effective_chlorine_trigger > 0 and pulses > 0 and is_chlorine_level_ok():
            ...
            send_server_event(
                ...
                payload={
                    ...
                    "acid_status": (
                        "low_level"
                        if not is_acid_level_ok()
                        else ("manual" if relay1_manual else ("running" if acid_pump_running else "automatic"))
                    ),
                    "chlorine_status": (
                        "low_level"
                        if not is_chlorine_level_ok()
                        else ("manual" if relay2_manual else ("running" if chlorine_pump_running else "automatic"))
                    ),
                    "acid_trigger_liters": round(effective_acid_trigger, 3),
                    "chlorine_trigger_liters": round(effective_chlorine_trigger, 3),
                    "acid_float_switch": "ON" if is_acid_level_ok() else "OFF",
                    "chlorine_float_switch": "ON" if is_chlorine_level_ok() else "OFF",
                },
            )

        last_time = now

    # Button control for Relay-3 with debounce/stable-press guard.
    button_state = GPIO.input(BUTTON_PIN)
    if button_state == GPIO.LOW:
        if button_prev_state == GPIO.HIGH:
            button_low_since = now
        if button_press_armed and (now - button_low_since) >= BUTTON_DEBOUNCE_SECONDS and (now - app_start_time) >= BUTTON_STARTUP_GUARD_SECONDS:
            if not relay3_running:
                start_relay3_cycle()
                button_press_armed = False
    else:
        button_press_armed = True
    button_prev_state = button_state

    if relay3_running and now >= relay3_off_time:
        GPIO.output(RELAY3, RELAY_OFF)
        relay3_running = False
    ...
    # Hard safety cut: if level becomes low, force OFF immediately
    if not is_acid_level_ok():
        GPIO.output(RELAY1, RELAY_OFF)
        acid_pump_running = False
        if relay1_manual:
    ...
    if not is_acid_level_ok():
        acid_status_value.configure(text="Low Level", text_color="#ff6b6b")
    elif relay1_manual:
    ...
    if not is_chlorine_level_ok():
        chlorine_status_value.configure(text="Low Level", text_color="#ff6b6b")
    elif relay2_manual:
