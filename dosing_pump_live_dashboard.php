...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    .content{margin-left:70px;transition:margin-left .3s ease}
    ...
    .meta{margin-top:8px;color:#88ffd8;font-size:14px}
    ...
    @media (max-width:640px){.sidebar{width:58px}.sidebar.expanded{width:220px}.content{margin-left:58px}.content.expanded{margin-left:220px}.wrap{width:calc(100vw - 10px);padding:5px}.grid{grid-template-columns:1fr}.title{font-size:30px}.bar-row{grid-template-columns:130px 1fr 70px}}
  </style>
</head>
<body>
...
async function loadData(){
  ...
  try{
    ...
    const snap = (currentMode === 'today') ? latest : (rangeLatest || {});
    const s = json.summary || {};
    const series = json.series || [];
    const rangeLabel = (json.range && json.range.label) ? json.range.label : currentMode;

    const waterSrc = s.water_source ? ` | Water Source: ${s.water_source}` : '';
    const chemSrc = s.chemical_source ? ` | Chemical Source: ${s.chemical_source}` : '';
    document.getElementById('meta').textContent = `Range: ${rangeLabel} | Server Time: ${json.server_time}${waterSrc}${chemSrc}`;
